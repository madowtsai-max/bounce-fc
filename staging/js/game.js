...
function showArenaUI() {
  ...
  btn.textContent = `KICK  ₦${formatBet(state.bet)}`;
}

// ── SCREEN TRANSITIONS ────────────────────────────────────
let _sessionBest = parseInt(localStorage.getItem('bounceFcBest') || '0');

function triggerBreach() {
  if (state.ngnEarned > _sessionBest) {
    _sessionBest = state.ngnEarned;
    localStorage.setItem('bounceFcBest', _sessionBest);
  }
  state.screen = 'breach';
  document.getElementById('breach-amount').textContent = `₦${state.ngnEarned.toLocaleString()}`;
  ...
  document.getElementById('stat-best').textContent = `₦${_sessionBest.toLocaleString()}`;
...
canvas.addEventListener('pointermove', e => {
  if (state.screen !== 'arena' || state.ballActive || e.buttons === 0) return;
  const p = getCanvasPos(e);
  state.isAiming = true;
  state.aimX = p.x;
  state.aimY = Math.min(p.y, PLAYER_Y - PLAYER_H - 10);
...
canvas.addEventListener('pointerup', () => {
  // aim stays locked — player taps KICK to fire
});
