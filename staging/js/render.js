...
function drawRoundBox() {
  ...
  const rx = cx - boxW / 2;
  ctx.beginPath();
  ctx.roundRect(rx, boxY, boxW, boxH, 6);
  ctx.fill();
  ctx.stroke();
  ...
  ctx.fillText(`ROUND ${state.round}`, cx - 38, boxY + 21);

  ctx.fillStyle = 'rgba(255,255,255,0.4)';
  ctx.fillText('│', cx, boxY + 21);

  ctx.font = 'bold 11px Arial';
  ctx.fillStyle = '#7fff7f';
  ctx.fillText(`₦${state.ngnEarned.toLocaleString()}`, cx + 42, boxY + 21);
  ctx.restore();
}
...
function render() {
  ...
  if (state.screen === 'arena' || state.screen === 'bet') {
    ...
    try {
      ...
      drawCombo();
      drawPopups();
      if (state.screen === 'arena') drawRoundBox();
    } finally {
      ctx.restore();
    }
  }
